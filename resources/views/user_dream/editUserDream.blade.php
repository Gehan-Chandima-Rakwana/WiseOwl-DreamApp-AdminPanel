  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>User Dream Edit</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">

              <li class="breadcrumb-item active">User Dream Edit</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="card card-primary">
            ...
            <div class="card-body">
              <div class="form-group">
                ...
                <input type="text" Disabled id="DreamID" class="form-control"  value="{{ $UserDream->user_dream_id }}">
              </div>
              <div class="form-group">
                ...
                <select class="form-control select2 " id="userID"  style="width: 100%;" autocomplete='off'>
                    ...
                    @foreach($user  as $user)
                          @if($user->user_id == $UserDream->user_id)
                            <option selected value="{{ $user->user_id }}">{{ $user->username }}</option>
                          @else
              ...
              <div class="form-group">
                ...
                <input type="text" id="DreamTitle" class="form-control" autocomplete='off' value="{{ $UserDream->dream_title }}" placeholder="Dream title">
              </div>
              <div class="form-group">
                ...
                <select class="form-control select2 " id="DreamType"  style="width: 100%;" autocomplete='off'>
                    ...
                    @foreach($dreamType  as $dreamType)
                          @if($dreamType->dream_type_id == $UserDream->dream_type_id)
              ...
              <div class="form-group">
                ...
                <textarea id="DreamStory" class="form-control" rows="20">{{ $UserDream->dream_story }}</textarea>
              </div>
              <div class="form-group">
                  <label>Tags.</label>
                  <select class="select2" id="Tags" multiple="multiple" data-placeholder="Select a State" style="width: 100%;">
                          @foreach($tag  as $tag)
                            @if(in_array($tag->tag_id, $SelectedUTagsArr))
                              <option selected value="{{ $tag->tag_id }}">{{ $tag->tag }}</option>
                            @else
              ...
              <div class="form-group">
                <label>Dream Date and time:</label>
                  <div class="input-group date" id="txtDreamDate" data-target-input="nearest">
                      <input type="text" id="DreamDate" value="{{ $UserDream->dream_datetime }}" class="form-control datetimepicker-input" data-target="#txtDreamDate"/>
              ...
              <div class="form-group">
                ...
                <select class="form-control select2 " id="DreamClarity"  style="width: 100%;" autocomplete='off'>
                    ...
                    @foreach($dreamClarity  as $dreamClarity)
                          @if($dreamClarity->sys_dream_clarity_id == $UserDream->sys_dream_clarity_id)
                            <option selected value="{{ $dreamClarity->sys_dream_clarity_id }}">{{ $dreamClarity->dream_clarity }}</option>
                          @else
              ...
              <div class="form-group">
                ...
                <select class="form-control select2 " id="DreamMood"  style="width: 100%;" autocomplete='off'>
                    ...
                    @foreach($dreamMood  as $dreamMood)
                          @if($dreamMood->sys_dream_mood_id == $UserDream->sys_dream_mood_id)
                            <option selected value="{{ $dreamMood->sys_dream_mood_id }}">{{ $dreamMood->dream_mood }}</option>
                          @else
              ...
              <div class="form-group">
                ...
                <select class="form-control select2 " id="SleepQuality"  style="width: 100%;" autocomplete='off'>
                    ...
                    @foreach($sleepQuality  as $sleepQuality)
                          @if($sleepQuality->sleep_quality_id == $UserDream->sys_sleep_quality)
                            <option selected value="{{ $sleepQuality->sleep_quality_id }}">{{ $sleepQuality->sleep_quality }}</option>
                          @else
              ...
              <div class="form-group">
                ...
                <select class="form-control select2 " id="Yamayan"  style="width: 100%;" autocomplete='off'>
                    ...
                    @foreach($sysYamayan  as $sysYamayan)
                          @if($sysYamayan->yamayan_id == $UserDream->sys_yamayan)
                            <option selected value="{{ $sysYamayan->yamayan_id }}">{{ $sysYamayan->yamayan_details }}</option>
                          @else
              ...
              <div class="form-group">
                  ...
                  <select class="select2" id="DreamMeanings" multiple="multiple" data-placeholder="Select a State" style="width: 100%;">
                          @foreach($DreamMeaning  as $DreamMeaning)
                            @if(in_array($DreamMeaning->dream_meaning_id, $selectedUDMarr))
  ...
  var Tags    = $("#Tags").val();
     ...
     var DreamMeanings  = $("#DreamMeanings").val(); 
        ...
        $.ajax({             
            type:"PUT",
            cache:false,
            url:"{{route('userDreamUpdateDetails')}}",  
